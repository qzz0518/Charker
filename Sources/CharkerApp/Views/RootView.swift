...
struct RootView: View {
    ...
    private var deviceChip: some View {
        ...
        .buttonStyle(.plain)
        .accessibilityLabel(Text(L10n.format("%@，%@", deviceTitle, model.snapshot.statusLabel)))
    ...
    private var deviceTitle: String {
        if model.snapshot.isDemo { return L10n.text("模拟充电器") }
        let fallback = model.hasRememberedCharger
            ? L10n.text("未连接")
            : L10n.text("查找充电器")
        let name = model.snapshot.displayName ?? fallback
        let prefix = "Anker "
    ...
    private var detail: some View {
        ZStack {
            ...
            case .dashboard: DashboardView(model: model).transition(sectionTransition)
            case .energy: EnergyHistoryView(model: model).transition(sectionTransition)
            case .devices: DevicesView(model: model).transition(sectionTransition)
