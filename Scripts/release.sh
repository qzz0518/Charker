#!/usr/bin/env bash
# Build the one canonical Charker release DMG used by GitHub, Sparkle and
# Homebrew. The source commit and tag must already exist; secrets stay in the
# login keychain and are referenced only by identity hash/profile name.
set -euo pipefail
...
DMG="$ROOT/dist/Charker-$VERSION.dmg"
